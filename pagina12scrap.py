...
class NewsSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=r'[4-4][0-9][0-9][0-9][0-9][0-9]+', deny='.+(/catamarca12|/dialogo).+',
                           deny_domains=['auth.pagina12.com.ar'], canonicalize=True,
                           deny_extensions=['7z', '7zip', 'apk', 'bz2', 'cdr,' 'dmg', 'ico,' 'iso,' 'tar', 'tar.gz',
                                            'pdf', 'docx', 'jpg', 'png', 'css', 'js']),
...
if __name__ == "__main__":
    urls = []
    for i in range(1, 2998):#2998 es el maximo
        urls.append("https://www.pagina12.com.ar/secciones/economia?page=" + str(i))

    crawler = CrawlerProcess()
    crawler.crawl(NewsSpider, save_pages_in_dir='.',  start_urls=urls)
    crawler.start()
